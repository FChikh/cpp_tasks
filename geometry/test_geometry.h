...
int testGeometry() {
    ...
    if (!abfcd.isConvex()) {
        std::cerr << "Test 3 failed. (convexity)\n";
        return 1;
    }

    Polygon bfced({b, f, c, e, d});
    if (bfced.isConvex()) {
        std::cerr << "Test 4 failed. (convexity)\n";
        return 1;
    }
    Triangle abd(a, b, d);
    Polygon abfced({a,b,f,c,e,d});
    if (!equals(abd.area() + bfced.area(), abfced.area())) {
        std::cerr << "Test 5 failed. (area)\n";
        return 1;
    }
    if (abd.isSimilarTo(abfced)) {
        std::cerr << "Test 6 failed. (similarity)\n";
        return 1;
    }
    if (abfced.isCongruentTo(abd)) {
        std::cerr << "Test 7 failed. (congruency)\n";
        return 1;
    }
    ...
    if (!equals(9 * abfced.area(), newAbfced.area())) {
        std::cerr << "Test 8 failed. (rotate, scale or area)\n";
        return 1;
    }
    if (!equals(3 * abfced.perimeter(), newAbfced.perimeter())) {
        std::cerr << "Test 9 failed. (rotate, scale or perimeter)\n";
        return 1;
    }
    ...
    if (!abfced.isSimilarTo(newAbfced)) {
        std::cerr << "Test 10 failed. (similarity)\n";
        return 1;
    }
    newAbfced.scale(a, 1. / 3);
    newAbfced.reflex(line1);
    if (!abfced.isCongruentTo(newAbfced)) {
        std::cerr << "Test 11 failed. (transformations or congruency)\n";
        return 1;
    }
    if (newAbfced == abfced) {
        std::cerr << "Test 12 failed. (transformations or equality)\n";
        return 1;
    }
    ...
    abd = Triangle(d, b, a);
    {
        ...
        if (!ok) {
            std::cerr << "Test 17 failed. (triangle tests)\n";
            return 1;
        }
    }
